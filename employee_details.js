...
function findEmployeeBySpecilization () {
    const foundSpecialization = employees.filter(employee => employee.specialization === 'Javscript');
    const specializationDisplay = foundSpecialization.map((employee, index) => `<p>${employee.name}: ${employee.department}: ${employee.specialization}</p>`).join('');
